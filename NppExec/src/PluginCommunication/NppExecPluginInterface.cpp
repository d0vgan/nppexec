...
namespace NppExecPluginInterface
{
    ...
    void CImpl::Enable(bool bEnable)
    {
        m_isEnabled = bEnable;
    }

    void CImpl::SetPluginName(const TCHAR* szNppExecDll)
    {
        m_sNppExecDll = szNppExecDll;
    }

    void CImpl::ProcessExternalPluginMsg(long nMsg, const TCHAR* srcModuleName, void* pInfo)
    {
        if ( !pInfo )
            return;
    ...
    void CImpl::NotifyExternalPluginResult(const tstr& id, DWORD dwResultCode)
    {
        if ( !id.IsEmpty() )
        {
    ...
    void CImpl::registerAsyncCmd(const tstr& id, const TCHAR* szScriptBody, AsyncCmd::eType type)
    {
        {
    ...
    void CImpl::registerPluginMsg(const CPluginMsg& msg)
    {
        if ( !msg.ID.IsEmpty() )
        {
            CCriticalSectionLockGuard lock(m_csPluginMsgs);
    ...
    DWORD WINAPI CImpl::BackgroundExecAsyncCmdThreadFunc(LPVOID lpParam)
    {
        CImpl* pImpl = (CImpl *) lpParam;
    ...
    DWORD WINAPI CImpl::BackgroundSendMsgThreadFunc(LPVOID lpParam)
    {
        CImpl* pImpl = (CImpl *) lpParam;
    ...
    void CImpl::initExecAsyncCmdThread()
    {
    ...
    void CImpl::stopExecAsyncCmdThread()
    {
        if ( !m_ExecAsyncCmdThreadDoneEvent.IsNull() )
        {
    ...
    DWORD CImpl::npemGetState() const
    {
    ...
    void CImpl::addCommand(CListT<tstr>& CmdList, tstr& Cmd)
    {
    ...
    void CImpl::getCmdListFromScriptBody(CListT<tstr>& CmdList, const TCHAR* szScriptBody)
    {
        tstr Line;
    ...
    void CImpl::npemExecuteScript(const tstr& id, const TCHAR* szScriptBody)
    {
        CListT<tstr> CmdList;
    ...
    void CImpl::npemExecuteCollateralScript(const tstr& id, const TCHAR* szScriptBody)
    {
    ...
    void CImpl::npemExecuteQueuedScript(const tstr& id, const TCHAR* szScriptBody)
    {
        // this will be executed asynchronously by its nature;
        // let's process it in a different thread to not block the message loop
    ...
    void CImpl::conPrintLine(tstr& Line)
    {
    ...
    void CImpl::npemPrint(const TCHAR* szText)
    {
        tstr Line;
